...
def piecewise_linear_prediction_function(xs, ys):
    def f(x):
        for x1, x2, y1, y2 in zip(
            [0] + list(xs), list(xs) + [1], [0] + list(ys), list(ys) + [1]
        ):
            if x <= x2:
...
def piecewise_objective_function(actual_scores, test_weights, num_points, xs_and_ys):
    return minimize(
        piecewise_linear_prediction_function(
            xs_and_ys[:num_points], xs_and_ys[num_points:]
        ),
        actual_scores,
        test_weights,
    ).fun
...
def piecewise_constraints(num_points):
    return [
        {"type": "ineq", "fun": lambda xs_and_ys: xs_and_ys[0]},
        *(
...
def optimize_piecewise_prediction_function(num_points, actual_scores, test_weights):
    return scipy.optimize.minimize(
        partial(piecewise_objective_function, actual_scores, test_weights, num_points),
...
def exponential_prediction_function(coefficients):
    return lambda s, d: 1 - sum(
        c * np.exp(-(k + 1) * s * d) for (k, c) in enumerate(coefficients)
    )
...
def exponential_objective_function(actual_scores, test_weights, coefficients):
    return minimize(
        exponential_prediction_function(coefficients), actual_scores, test_weights
    ).fun


def optimize_exponential_prediction_function(
    num_coefficients, actual_scores, test_weights
):
    return scipy.optimize.minimize(
        partial(exponential_objective_function, actual_scores, test_weights),
