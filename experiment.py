        ...
        for i in range(students):
            for j in range(tests):
                if actual_scores[i][j] is not None and not np.isnan(
                    ...
                    total += (
                        test_weights[j]
                        * (f(scores[i], difficulties[j]) - actual_scores[i][j]) ** 2
                    )
        return total
...
def minimize(f, actual_scores, test_weights, **kwargs):
    ...
    return scipy.optimize.minimize(
        objective(f, actual_scores, test_weights),
        x0,
        bounds=bounds,
...
candidate_functions = {
    ...
    "RELU(8)": relu(8),
    "RELU(16)": relu(16),
    "Smoothstep 3": lambda s, d: (
        lambda x: 0 if x <= 0 else 3 * x ** 2 - 2 * x ** 3 if x <= 1 else 1
    )(s * d),
    "Smoothstep 5": lambda s, d: (
        lambda x: 0
        if x <= 0
        else 6 * x ** 5 - 15 * x ** 4 + 10 * x ** 3
        if x <= 1
        else 1
    )(s * d),
    "Smoothstep 7": lambda s, d: (
        lambda x: 0
        if x <= 0
        else -20 * x ** 7 + 70 * x ** 6 - 84 * x ** 5 + 35 * x ** 4
        if x <= 1
        else 1
    )(s * d),
    "Smoothstep 9": lambda s, d: (
        lambda x: 0
        if x <= 0
        else 70 * x ** 9 - 315 * x ** 8 + 540 * x ** 7 - 420 * x ** 6 + 126 * x ** 5
        if x <= 1
        else 1
    )(s * d),
}
...
def apply_all_candidate_functions(actual_scores, test_weights):
    return dict(
        (description, minimize(function, actual_scores, test_weights).fun)
        for (description, function) in candidate_functions.items()
    )
...
def random_trial(students, tests, probability_of_writing_test):
    ...
    not_written = np.random.uniform(0, 1, (students, tests)) < (
        1 - probability_of_writing_test
    )
...
weights = np.array(
    [1 / 6, 1 / 15, 1 / 30, 1 / 30, 1 / 30, 1 / 9, 1 / 9, 1 / 9, 1 / 9, 2 / 9]
)
